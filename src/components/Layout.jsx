...
export default function Layout() {
  ...
  const navItems = [
    ...
    { label: 'Data Upload', icon: 'upload_file', path: '/upload' },
    { label: 'LCA', icon: 'eco', path: '/lca' },
    { label: 'Circularity', icon: 'rebase_edit', path: '/circularity' },
    { label: 'Transportation', icon: 'local_shipping', path: '/transportation' },
    { label: 'Simulator', icon: 'precision_manufacturing', path: '/simulator' },
    { label: 'Scoring', icon: 'analytics', path: '/scoring' },
  ...
  return (
    <div className="flex min-h-screen bg-background font-body-md text-on-surface">
      ...
      >
        {/* Sidebar Header with Collapse Arrow Button */}
        <div className="flex items-center justify-between mb-xl px-sm">
          <div className="flex items-center gap-sm">
            ...
            <div>
              <h1 className="font-headline-md text-headline-md font-bold text-primary">EcoMetrix AI</h1>
              <p className="font-label-md text-label-md text-on-surface-variant">Carbon Intelligence</p>
            </div>
          </div>

          {/* Desktop Arrow Toggle Button (Collapse Sidebar) */}
          <button
            onClick={() => setSidebarCollapsed(true)}
            className="hidden md:flex items-center justify-center w-8 h-8 rounded-full bg-surface-container hover:bg-surface-container-high text-on-surface-variant hover:text-primary transition-colors cursor-pointer"
        ...
        <div className="pt-md border-t border-outline-variant flex items-center justify-between px-sm">
          <div className="flex items-center gap-sm">
            ...
            <div className="overflow-hidden">
              <p className="text-xs font-semibold text-on-surface truncate">{user?.name || 'Engineer'}</p>
              <p className="text-[10px] text-on-surface-variant capitalize">{user?.role || 'engineer'}</p>
            </div>
          </div>
          <button
          ...
          <div className="flex items-center gap-md">
            ...
            {sidebarCollapsed && (
              <button
                ...
                className="hidden md:flex items-center gap-1.5 px-3 py-1.5 bg-surface-container hover:bg-surface-container-high text-primary rounded-md border border-outline-variant transition-all font-semibold text-xs cursor-pointer shadow-sm"
            ...
            <nav className="flex items-center gap-md font-label-md text-label-md">
              <span className="text-primary font-bold">{user?.material || 'Aluminium'}</span>
              <span className="text-outline-variant">/</span>
              <span className="text-on-surface-variant">{user?.plant || 'Plant A-12'}</span>
              <span className="text-outline-variant">/</span>
              <span className="text-on-surface-variant capitalize">{user?.role || 'Engineer'}</span>
            </nav>
          </div>
          ...
          <div className="flex items-center gap-sm relative">
            ...
            <div className="relative">
              <button
                ...
                className="p-sm text-on-surface-variant hover:text-primary rounded-full hover:bg-surface-container-low transition-all relative cursor-pointer"
              >
                ...
                <div className="absolute right-0 mt-2 w-72 bg-surface-container-lowest border border-outline-variant rounded-lg shadow-lg p-md z-50">
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="p-2 bg-surface-bright rounded border border-outline-variant">
                      <p className="font-semibold text-error">Hotspot Alert</p>
                      <p className="text-on-surface-variant">Metal production exceeding threshold at 42% intensity.</p>
                    </div>
                    <div className="p-2 bg-surface-bright rounded border border-outline-variant">
                      <p className="font-semibold text-primary">Data Processed</p>
                      <p className="text-on-surface-variant">Q3 Emissions sheet uploaded successfully.</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="relative">
              <button
                ...
                className="p-sm text-on-surface-variant hover:text-primary rounded-full hover:bg-surface-container-low transition-all flex items-center gap-1 cursor-pointer"
              >
      ...
      {/* Floating Eco Assistant AI Chatbot */}
      <Chatbot />
    </div>
  );
}
